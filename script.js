...
/* ═══════════════════════════════════════════
   IMAGE PROBE UTILITY
   Tries .jpg, .jpeg, .png, .webp for each index.
...
async function probeImageFolder(folder, prefix) {
  const exts = ['jpg', 'jpeg', 'png', 'webp'];
  const found = [];
  for (let i = 1; ; i++) {
    let url = null;
    for (const ext of exts) {
      const candidate = `${folder}/${prefix}_${i}.${ext}`;
      const result = await _probeImg(candidate);
      if (result) { url = result; break; }
    }
    if (!url) break;
    found.push(url);
  }
...
/* ═══════════════════════════════════════════
   4. SLIDER — images auto-loaded from /slider1/
═══════════════════════════════════════════ */
let Slider = {};
let _sliderReadyResolve;
const _sliderReady = new Promise(res => { _sliderReadyResolve = res; });
...
(async function initSlider() {
  ...
  if (!track) { _sliderReadyResolve({}); return; }

  /* ── Probe images from slider1/ folder ── */
  const imageUrls = await probeImageFolder('slider1', 'slider1_image');

  /* ── Build slide items into the DOM ── */
    ...
    item.innerHTML =
      `<div class="slide-card">` +
        `<div class="slide-inner">` +
          ...
          `<div class="slide-overlay"><span class="slide-label">Photo ${i + 1}</span></div>` +
        `</div>` +
      `</div>`;
    track.appendChild(item);
  });

  /* Fallback: show a placeholder message if no images found */
  if (imageUrls.length === 0) {
    track.innerHTML = `<div class="slide-item" data-index="0" style="display:flex;align-items:center;justify-content:center;min-width:175px;aspect-ratio:2/3;border:1.5px dashed rgba(125,249,255,0.3);border-radius:20px;"><span style="color:rgba(125,249,255,0.5);font-size:0.8rem;text-align:center;padding:20px;">Thêm ảnh vào<br/>thư mục slider1/</span></div>`;
  }
  ...
  */
  ...
  const api = { goTo, shift, goToReal, get centerIndex() { return centerIndex; } };
  Object.assign(Slider, api);
  _sliderReadyResolve(api);
})();

/* ═══════════════════════════════════════════
   5. MOUSE PARALLAX TILT ON SLIDE CARDS
═══════════════════════════════════════════ */
_sliderReady.then(function() {
...
(function initWheelScroll() {
  const viewport = document.getElementById('sliderViewport');
  if (!viewport || !Slider.shift) return;
...
(function initOceanCanvas() {
  ...
  const JELLY_PALETTES = [
    { bell: [180, 80,220], glow: 'rgba(200,100,255,0.55)' },
    { bell: [50, 210,230], glow: 'rgba(60,230,255,0.55)'  },
    { bell: [90, 150,255], glow: 'rgba(110,170,255,0.55)' },
    { bell: [220, 90,170], glow: 'rgba(255,110,200,0.55)' },
    { bell: [60, 220,180], glow: 'rgba(80,255,200,0.50)'  },
  ];
  function makeJellyfish() {
    const pal = JELLY_PALETTES[Math.floor(Math.random() * JELLY_PALETTES.length)];
    const [br, bg, bb] = pal.bell;
    return {
      x:              rand(0.06, 0.94),
      y:              rand(0.56, 0.90),
      worldDepth:     rand(0.78, 1.00),
      vx:             rand(0.00007, 0.00017) * (Math.random() > 0.5 ? 1 : -1),
      vy:             rand(0.00003, 0.00011) * (Math.random() > 0.5 ? 1 : -1),
      size:           rand(16, 36),
      phase:          rand(0, Math.PI * 2),
      phaseSpeed:     rand(0.022, 0.050),
      wanderTimer:    rand(0, 140),
      wanderInterval: rand(80, 200),
      bellR: br, bellG: bg, bellB: bb,
      glowColor: pal.glow,
      numTentacles: Math.floor(rand(5, 9)),
    };
  }
  const JELLYFISH = Array.from({ length: 8 }, makeJellyfish);
  ...
  function drawJellyfish() {
    ...
    JELLYFISH.forEach(j => {
      /* wander: periodically pick a new random drift direction */
      j.wanderTimer++;
      if (j.wanderTimer >= j.wanderInterval) {
        j.wanderTimer    = 0;
        j.wanderInterval = rand(80, 220);
        j.vx = rand(0.00007, 0.00020) * (Math.random() > 0.5 ? 1 : -1);
        j.vy = rand(0.00003, 0.00013) * (Math.random() > 0.5 ? 1 : -1);
      }
      ...
      /* soft boundary bounce — keep in lower portion */
      if (j.x < 0.05) { j.x = 0.05; j.vx =  Math.abs(j.vx); }
      if (j.x > 0.95) { j.x = 0.95; j.vx = -Math.abs(j.vx); }
      if (j.y < 0.53) { j.y = 0.53; j.vy =  Math.abs(j.vy); }
      if (j.y > 0.93) { j.y = 0.93; j.vy = -Math.abs(j.vy); }

      const localA = depthAlpha(sp, j.worldDepth - 0.18, j.worldDepth, j.worldDepth + 0.18);
      if (localA <= 0) return;

      const jx   = j.x * W;
      const jy   = j.y * H;          // bell opening (bottom of dome)
      const s    = j.size;
      const pulse = 0.86 + 0.14 * Math.sin(j.phase); // rhythmic contraction
      const sw   = s * pulse;         // bell half-width
      const sh   = s * 0.62 * pulse;  // bell height
      const finalA = localA * a * 0.88;
      ...
      /* ── bell dome ── */
      const bellGrd = ctx.createRadialGradient(jx, jy - sh * 0.45, 0, jx, jy - sh * 0.15, sw * 1.05);
      bellGrd.addColorStop(0,   `rgba(${j.bellR},${j.bellG},${j.bellB},0.60)`);
      bellGrd.addColorStop(0.55,`rgba(${j.bellR},${j.bellG},${j.bellB},0.32)`);
      bellGrd.addColorStop(1,   `rgba(${j.bellR},${j.bellG},${j.bellB},0.07)`);
      ctx.fillStyle = bellGrd;
      ctx.beginPath();
      ctx.ellipse(jx, jy, sw, sh, 0, 0, Math.PI, true); // top half = dome curves up
      ctx.closePath();
      ctx.fill();

      /* ── bell rim stroke ── */
      ctx.shadowBlur  = 12;
      ctx.strokeStyle = `rgba(${j.bellR},${j.bellG},${j.bellB},0.55)`;
      ctx.lineWidth   = 1.0;
      ctx.beginPath();
      ctx.ellipse(jx, jy, sw, sh, 0, 0, Math.PI, true);
      ctx.closePath();
      ctx.stroke();

      /* ── inner highlight (sub-umbrella glow) ── */
      ctx.shadowBlur = 0;
      ctx.fillStyle  = `rgba(255,255,255,0.07)`;
      ctx.beginPath();
      ctx.ellipse(jx, jy - sh * 0.28, sw * 0.52, sh * 0.38, 0, 0, Math.PI, true);
      ctx.closePath();
      ctx.fill();

      /* ── oral arms (short frilly curtain just below rim) ── */
      ctx.shadowBlur  = 6;
      ctx.strokeStyle = `rgba(${j.bellR},${j.bellG},${j.bellB},0.30)`;
      ctx.lineWidth   = 1.4;
      const armCount  = 4;
      for (let ai = 0; ai < armCount; ai++) {
        const ax = jx + (ai / (armCount - 1) - 0.5) * sw * 1.2;
        const len = s * (0.38 + 0.12 * Math.sin(j.phase + ai * 1.1));
        ctx.beginPath();
        ctx.moveTo(ax, jy);
        for (let seg = 1; seg <= 5; seg++) {
          const frac = seg / 5;
          const wx   = ax + Math.sin(j.phase * 1.5 + seg * 1.2 + ai * 2.1) * s * 0.13;
          ctx.lineTo(wx, jy + frac * len);
        }
        ctx.stroke();
      }
      ...
      ctx.shadowBlur = 10;
      for (let ti = 0; ti < j.numTentacles; ti++) {
        const spread = sw * 1.55;
        const tx     = jx + (ti / (j.numTentacles - 1) - 0.5) * spread;
        const tLen   = s * (2.0 + 0.6 * Math.sin(j.phase * 0.8 + ti * 0.85));
        const opacity = 0.30 + 0.15 * Math.sin(ti * 1.3);
        ctx.strokeStyle = `rgba(${j.bellR},${j.bellG},${j.bellB},${opacity})`;
        ctx.lineWidth   = 0.7 + 0.35 * Math.sin(ti * 0.9);
        ctx.beginPath();
        ctx.moveTo(tx, jy);
        for (let seg = 1; seg <= 10; seg++) {
          const frac = seg / 10;
          const wx   = tx + Math.sin(j.phase * 1.2 + seg * 0.95 + ti * 1.4) * s * 0.20;
          const wy   = jy + frac * tLen;
          ctx.lineTo(wx, wy);
        }
        ctx.stroke();
      }
...
(function initLightbox() {
  ...
  function openLightbox(src, alt, label) {
    // Load full-resolution version by bumping Unsplash width param
  ...
  function closeLightbox() {
    ...
    document.body.style.overflow = '';
    // Delay src clear until transition ends
    setTimeout(() => { lightboxImg.src = ''; }, 350);
  }

  // Delegate click on slide items (original slider)
  ...
  // Delegate click on slider43 cards
  document.addEventListener('click', (e) => {
...
/* ═══════════════════════════════════════════
   ...
   Images auto-loaded from /slider2/ folder
═══════════════════════════════════════════ */
(async function initSlider43() {
  ...
  /* ── Probe images from slider2/ folder ── */
  const rawUrls = await probeImageFolder('slider2', 'slider2_image');
  ...
  const IMAGES = rawUrls.length > 0
    ? rawUrls.map((url, i) => ({
        url,
        label: `Photo ${i + 1}`,
        color: i % 2 === 0 ? '#7df9ff' : '#4a9eff'
      }))
    : [{ url: '', label: 'Thêm ảnh vào thư mục slider2/', color: '#7df9ff' }];
